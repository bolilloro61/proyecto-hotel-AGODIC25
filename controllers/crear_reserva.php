...
try {
    ...
    // Preparar y enviar correo de confirmación (usar SMTP/PHPMailer si está configurado)
    try {
        $to = filter_var($email, FILTER_VALIDATE_EMAIL) ? $email : null;
        if (!$to) {
            @file_put_contents(__DIR__ . '/../logs/error_log.txt', date('c') . " - crear_reserva: invalid email provided: " . json_encode($email) . "\n", FILE_APPEND);
        } else {
            ...
            if (!empty($mailConfig['use_smtp'])) {
                ...
                if (file_exists($autoload)) {
                    try {
                        ...
                        $mail->Body = $html;
                        // Alt body
                        $alt = trim(strip_tags(str_replace(array("\n", "<br>", "<br/>", "</p>"), "\n", $html)));
                        $mail->AltBody = $alt;
                        $sent = (bool)$mail->send();
                        if (!$sent) {
                            @file_put_contents(__DIR__ . '/../logs/error_log.txt', date('c') . " - crear_reserva: PHPMailer->send() returned false for {$to} - reserva {$reservaId}\n", FILE_APPEND);
                        }
                    } catch (Exception $ex) {
                        @file_put_contents(__DIR__ . '/../logs/error_log.txt', date('c') . " - crear_reserva: PHPMailer exception: " . $ex->getMessage() . "\n", FILE_APPEND);
                        $sent = false;
                    }
                } else {
                    @file_put_contents(__DIR__ . '/../logs/error_log.txt', date('c') . " - crear_reserva: vendor/autoload.php not found, install PHPMailer via composer\n", FILE_APPEND);
                }
            }

            // Si no se envió por SMTP/PHPMailer, intentar fallback a mail()
            if (!$sent) {
                ...
                if (!$fallback) {
                    @file_put_contents(__DIR__ . '/../logs/error_log.txt', date('c') . " - crear_reserva: mail() fallback failed for {$to} - reserva {$reservaId}\n", FILE_APPEND);
                }
            }
        }
    } catch (Exception $e) {
        @file_put_contents(__DIR__ . '/../logs/error_log.txt', date('c') . " - crear_reserva: mail exception: " . $e->getMessage() . "\n", FILE_APPEND);
    }
